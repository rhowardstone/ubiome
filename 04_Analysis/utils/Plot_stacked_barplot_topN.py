...
if __name__ == "__main__":
	...
	order = [r[0] for r in sorted(temp, key=lambda x:float(x[2]))][::-1]
	toOther = False
	if len(order) > topN:
		print("Total number of taxonomic groups:", len(order), ">", topN)
		order = order[:topN]
		toOther = True
	...
	tax_order = [ele for ele in tax_order if ele!="Other"] + ['Other']
	...
	if isSep == '1' or isWild == '1':
		...
		ax.annotate('', xy=(0.025, -0.08), xycoords='axes fraction', xytext=(0.49, -0.08), arrowprops=dict(arrowstyle="<-", color=tmp_cols[0], lw=2, mutation_scale=30, mutation_aspect=2))
		ax.annotate('', xy=(0.51, -0.08), xycoords='axes fraction', xytext=(0.975, -0.08), arrowprops=dict(arrowstyle="->", color=tmp_cols[1], lw=2, mutation_scale=30, mutation_aspect=2))
		ax.annotate('La Jolla', xy=(0.25,-0.13), xycoords='axes fraction', color=tmp_cols[0], fontsize=32, fontweight='bold', horizontalalignment='center')
		ax.annotate('Groton', xy=(0.75,-0.13), xycoords='axes fraction', color=tmp_cols[1], fontsize=32, fontweight='bold', horizontalalignment='center')
	...
	#ax.set_ylabel("")
	plt.xticks(fontsize=32, fontweight='bold', horizontalalignment='center')
	ax.set_ylim((0,1))
	...
	if toLegend == '1':
		...
		if not toOther:
			# Remove "Other" from handles and labels
			if "Other" in labels:
				index = labels.index("Other")
				handles.pop(index)
				labels.pop(index)
		legend = ax.legend(reversed(handles), reversed(labels), fontsize=14, loc='upper center', bbox_to_anchor=(0.5, -0.2), ncol=3)  #outside below centered
	...
	if GROUP.find('ABX')!=-1:
		...
		# Print percentages for each taxa, day, and site
	print("\nRelative Abundances:")
	print("Site\tDay\tTaxon\tPercentage")
	print("-" * 50)

	# Split into sites if separated
	if isSep == '1':
		sites = ['All']
	else:
		sites = ['La Jolla', 'Groton']
		split_point = len(Columns) // 2
	
	for site_idx, site in enumerate(sites):
		if isSep == '1':
			col_range = range(len(Columns))
		else:
			col_range = range(split_point) if site_idx == 0 else range(split_point, len(Columns))
			
		for i in col_range:
			print(f"\n--- {site} - {Columns[i]} ---")
			total = 0
			for tax in tax_order:
				percentage = vectors[tax][i] * 100  # Convert to percentage
				print(f"{site}\t{Columns[i]}\t{tax}\t{percentage:.2f}%")
				total += percentage
			print(f"{site}\t{Columns[i]}\tTotal\t{total:.2f}%")
			print("-" * 50)
