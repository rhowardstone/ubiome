...
def find_amplicons(ID, sequence, primers, m_mismatches):
    ...
    for i in range(len(strand)-1):
        if points[strand[i]] == '27f' and points[strand[i+1]] == 'R':
            n+=1
            #interval = str(strand[i])+'-'+str(strand[i+1])
            interval = str(strand[i]+len('AGRRTTYGATYHTDGYTYAG'))+'-'+str(strand[i+1])
    ...
    for i in range(len(strand)-1):
        if points[strand[i]] == '27f' and points[strand[i+1]] == 'R':
            n += 1
            #interval = str(strand[i])+'-'+str(strand[i+1])
            interval = str(strand[i])+'-'+str(strand[i+1]+len('AGTACYRHRARGGAANGR'))
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Error - usage:\n >>> python3 Extract_StrainID_regions_par_mem.py [input_directory] [m_mismatches] [output_directory]\n")
        exit()
    
    input_dir, m_mismatches, output_dir = sys.argv[1:]
